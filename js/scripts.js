...
-------------------------------------------------------------- */
    ...
    $( function() {
        ...
        const mainNavbar = document.querySelector('.tc-navbar-st1');
        const smoothWrapper = document.getElementById('smooth-wrapper');
        if (mainNavbar && smoothWrapper && mainNavbar.closest('#smooth-content')) {
        ...
        const closeOpenNavigation = () => {
            ...
            if (openCollapse && window.bootstrap) {
                const instance = bootstrap.Collapse.getInstance(openCollapse);
                if (instance) instance.hide();
            }
        };

        window.liniyaDomScrollToHash = function (hash, updateHash = true) {
            ...
            const fallbackScroll = () => {
                const targetTop = target.getBoundingClientRect().top + window.pageYOffset - navbarHeight - 20;
                window.scrollTo({ top: Math.max(targetTop, 0), behavior: 'smooth' });
            };

            if (smoother) {
                try {
                    smoother.scrollTo(target, true, `top ${navbarHeight + 20}px`);
                } catch (error) {
                    fallbackScroll();
                }
            } else {
            ...
            if (updateHash && window.history && window.location.hash !== hash) {
        ...
        document.querySelectorAll('a[href^="#"], a[href^="index.html#"]').forEach(link => {
            link.addEventListener('click', function (event) {
                if (this.dataset.bsToggle || this.dataset.bsTarget) return;

                const href = this.getAttribute('href');
                const hash = href.includes('#') ? href.slice(href.indexOf('#')) : '';
                const isSamePageIndexLink = href.startsWith('index.html#') && /index\.html?$/.test(window.location.pathname.split('/').pop() || 'index.html');

                if (href.startsWith('#') || isSamePageIndexLink) {
                    event.preventDefault();
                    window.liniyaDomScrollToHash(hash);
                }
            });
        });
        ...
        if (window.location.hash) {
            window.setTimeout(() => {
                window.liniyaDomScrollToHash(window.location.hash, false);
            }, 350);
        }
        ...
        if (projectForm) {
            ...
            const submitText = submitButton ? submitButton.querySelector('.txt span') : null;
            const submitHover = submitButton ? submitButton.querySelector('.txt') : null;
            const originalText = submitText ? submitText.textContent : '';

            projectForm.addEventListener('submit', function (event) {
                event.preventDefault();

                const requiredFields = Array.from(projectForm.querySelectorAll('[required]'));
                const invalidFields = requiredFields.filter(field => !field.value.trim());

                projectForm.querySelectorAll('.is-invalid').forEach(field => {
                ...
                if (invalidFields.length) {
                    ...
                    if (status) {
                        ...
                        status.textContent = 'Пожалуйста, заполните обязательные поля: имя, телефон и тип проекта.';
                    }
                ...
                if (submitHover) {
                    submitHover.setAttribute('data-text', 'Отправляем...');
                }

                window.setTimeout(() => {
                    const formData = new FormData(projectForm);
                    const requestData = Object.fromEntries(formData.entries());
                    window.localStorage.setItem('liniyaDomLastRequest', JSON.stringify({
                        ...requestData,
                    ...
                    if (status) {
                        ...
                        status.textContent = 'Спасибо. Заявка сохранена на сайте, и мы свяжемся с вами для уточнения деталей проекта.';
                    }
                    if (submitButton) {
                    ...
                    if (submitHover) {
                        submitHover.setAttribute('data-text', originalText.trim());
                    }
                }, 700);
            });
        }
